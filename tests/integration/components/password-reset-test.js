...
module('Integration | Component | password reset', function(hooks) {
  setupTest(hooks);
  ...
  test('tapping button fires an external action', function(assert) {
    ...
    var component = this.owner.factoryFor('component:password-reset').create();
    run(function() {
      component.set("password", "test");
      assert.equal(component.validate()[0].string, "Passwords must be greater than or equal to 6", "Validate Password");
